...
if Settings['EmailLog']:
    try:
        ...
        EmailContents.append("----------------------------------------------------------------------------")
        try:
            yag = yagmail.SMTP(Settings['EmailUsername'], Settings['EmailPassword'])
            yag.send(Settings['EmailRecipient'], 'Plex-Cleaner.py Log', EmailContents)
            log("")
            log("Email of log file contents sent successfully.")
        except:
            log("Could not send email.  Please ensure a username, password, and recipient are specified in your Config file.")
    except:
        log("Unable to send log via email.  Please install yagmail using the command 'pip install yagmail[all]' ...")